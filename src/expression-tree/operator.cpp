...
void Operator::create(TOperator array[OPERATORS_SIZE])
{
    ...
    TOperator unaryPlus;
    unaryPlus.value = operators.at(UNARY_PLUS);
    unaryPlus.id = UNARY_PLUS;
    unaryPlus.priority = priorities.at(UNARY_PLUS);
    unaryPlus.associativity = LEFT;
    array[UNARY_PLUS] = unaryPlus;
...
std::string Operator::join()
{
    return std::string({ operators.at(ADDITION) })
        ...
        + std::string({ operators.at(UNARY_PLUS) })
